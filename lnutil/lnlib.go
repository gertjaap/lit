...
func HTLCScript(
	...
	absDelay, relDelay uint16, sender bool) []byte {
	...
	if sender {
		...
		builder.AddData(myPubkey[:]) // our pubkey; we can spend right away
	}
	...
	if sender {
		...
		builder.AddData(theirPubkey[:]) // our pubkey; we can spend right away
	}
